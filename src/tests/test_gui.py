...
class LipidDetailsScreenTests(unittest.TestCase):
    ...
    def test_back_button_is_disabled(self):
        ...
        # Verify that we are now on FilePickerScreen
        self.assertIs(self.form.centralWidget().nav_buttons.btn_back.isEnabled(), False)
...
class FilePickerScreenTests(unittest.TestCase):
    ...
    def test_next_button_opens_input_summary_screen(self):
        QTest.mouseClick(self.form.centralWidget().nav_buttons.btn_next, Qt.LeftButton)
        self.assertIs(type(self.form.centralWidget()), type(InputSummaryScreen()))

    def test_back_button_opens_lipid_details_screen(self):
        QTest.mouseClick(self.form.centralWidget().nav_buttons.btn_back, Qt.LeftButton)
        self.assertIs(type(self.form.centralWidget()), type(LipidDetailsScreen()))
